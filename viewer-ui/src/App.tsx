...
function App() {
  ...
  useEffect(() => {
    ...
    if (isBrowserRuntime) {
      if (isRemoteViewerRuntime) {
        // remoteViewer dispatches sprites+layout (via cached mock payload)
        // on every bootstrap, then forwards live agent events from SSE.
  ...
  return (
    <div ref={containerRef} className="w-full h-full relative overflow-hidden">
      ...
      {isRemoteViewerRuntime && (
        <>
          <button
            type="button"
            onClick={() => setIsReplayOpen(true)}
            className="fixed top-3 right-3 z-40 bg-bg border-2 border-border text-accent-bright text-sm px-3 py-1 rounded-none shadow-pixel hover:bg-btn-hover"
          >
            replay
          </button>
          <ReplayPanel isOpen={isReplayOpen} onClose={() => setIsReplayOpen(false)} />
        </>
      )}
    </div>
  );
